...
# template placeholder, empty required fields, length limits, keyword/URL
# shape, category ids, review-info completeness and image sizes.
#
# Usage:
...
# iOS App Store screenshot sizes deliver accepts (width x height).
...
check_android() {
  ...
  [ "$HAVE_IMAGE_TOOL" -eq 1 ] || return 0
  local icon="$android_dir/images/icon.png"
  if [ -f "$icon" ]; then
    local wh
    wh="$(get_wh "$icon")" && {
  ...
  local feature="$android_dir/images/featureGraphic.png"
  if [ -f "$feature" ]; then
    local wh
    wh="$(get_wh "$feature")" && {
  ...
  local phone_dir="$android_dir/images/phoneScreenshots"
  if [ -d "$phone_dir" ]; then
    local count=0 img
    while IFS= read -r -d '' img; do
      case "$(basename "$img")" in .gitkeep) continue ;; esac
      ...
      w="${wh%% *}"
      h="${wh##* }"
      if [ "$w" -lt 320 ] || [ "$w" -gt 3840 ] || [ "$h" -lt 320 ] || [ "$h" -gt 3840 ]; then
        add_violation "$(rel "$img")" "${w}x${h} is outside the 320-3840px range"
      fi

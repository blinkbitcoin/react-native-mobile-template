...
# Version: HEAD tag vX.Y.Z → X.Y.Z; else a "chore(main): release X.Y.Z" release commit
#          (HEAD's own subject, or its second parent's when the release PR was
#          merged with a merge commit) → X.Y.Z; else open release-please PR
#          ("chore(main): release X.Y.Z") → X.Y.Z; else last tag patch+1.
...
RELEASE_SUBJECT_RE='^chore\(main\): release [0-9]+\.[0-9]+\.[0-9]+'
...
release_subject() {
  ...
  for rev in HEAD 'HEAD^2'; do
    ...
    if printf '%s' "$subject" | grep -qE "$RELEASE_SUBJECT_RE"; then
      printf '%s' "$subject"
      return 0
    fi
  done

...
starts a workflow from an event that token caused. So `release-beta.yml` and
`web.yml`, both triggered on `release: published`, never fired: `v0.2.2`
...
`release-please.yml` starts the follow-on work itself, at the tag, with the
default token. No App, no secrets.

- `.github/workflows/release-please.yml` — after `release_created`,
  `gh workflow run release-beta.yml --ref $TAG -f tag=$TAG` and
  `gh workflow run web.yml --ref $TAG -f deploy=true`; after `prs_created`,
...
- `.github/workflows/release-beta.yml`, `web.yml` — `workflow_dispatch` only;
...
commit, which is what `release-retry.yml` matches a failed beta run on.
...
keeps using `github-release.yml`'s optional secrets in shared-workflows.

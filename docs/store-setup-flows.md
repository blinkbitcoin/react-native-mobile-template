...
- `.github/workflows/cd-store-listing.yml` is the dispatchable workflow behind the `store-metadata` skill: it runs the same `sync_metadata` and `pull_metadata` lanes from Actions, on the `production` environment and behind its reviewers.

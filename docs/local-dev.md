...
```sh
# iOS simulator
xcrun simctl openurl booted "rnmt://expo-development-client/?url=http%3A%2F%2Flocalhost%3A8081"

# Android emulator
adb shell am start -a android.intent.action.VIEW \
  -d "rnmt://expo-development-client/?url=http%3A%2F%2F10.0.2.2%3A8081"
```
...
- `scripts/e2e/maestro-android.sh` runs `adb reverse tcp:8081 tcp:8081` and
  `adb reverse tcp:4000 tcp:4000` so Metro and the mock API are reachable by
